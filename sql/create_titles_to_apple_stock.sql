CREATE TABLE cs230_finance_data.titles_10_to_apple_buckets WITH (
    format = 'PARQUET',
...
FROM
    ...
    cs230_finance_data.apple_stock_bucketized t1 ON t2.clean_date = t1.date -- Group by the date from the prices table AND the new article group
GROUP BY
    t1.date,
    t2.article_group