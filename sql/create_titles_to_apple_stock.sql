CREATE TABLE cs230_finance_data.titles_10_to_apple_buckets
WITH (
    format = 'PARQUET',
    external_location = 's3://cs230-market-data-2025/titles_10_to_apple_buckets'
)
AS
WITH
    T2_ARTICLES AS (
...
SELECT
    ...
    ANY_VALUE(t1.volume_change_ratio) as volume_change_ratio,
    ANY_VALUE(t1.volume_change_ratio_decile_bucket) as volume_change_ratio_decile_bucket,
    ANY_VALUE(t1.volume_delta) as volume_delta,
    ...
    -- Other t2 columns (article_stock_symbol, url, publisher) are dropped
    -- as requested by grouping
FROM
    T2_ARTICLES t2
LEFT JOIN
    -- Join directly to the table instead of using the CTE
    cs230_finance_data.apple_stock_bucketized t1 ON t2.clean_date = t1.date
-- Group by the date from the prices table AND the new article group
GROUP BY
    t1.date, t2.article_group