# -*- coding: utf-8 -*-
"""A_sen_embed_transformer_cofinetune_v0_1epoch_keep.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1DkSHxmuicmKFYV3vO5QrhbhhYaO_V8y9
...
"""# Task
Configure an `AdamW` optimizer with differential learning rates, assigning `1e-5` to the unfrozen FinBERT parameters and `1e-3` to the `TimeSeriesTransformer` and other components. Train the `FinBERT_TimeSeries_Model` for 5 epochs using the `train_loader` and `test_loader`. Calculate and print the Mean Squared Error (MSE), Mean Absolute Error (MAE), and R-squared (R²) metrics for the test set, and plot the training and validation loss curves to visualize convergence.
