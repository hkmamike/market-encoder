# -*- coding: utf-8 -*-
"""cleaned: A_non_transformer_keep_withbacktest.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1XlE-0jrnjMaOumd8yhhbJEmE7XV97BPl
...
relative_signal_raw = np.sign(pred_future - pred_past)

# Modify: If the signal is to short (-1), change it to 0 (stay out of market)
relative_signal = np.where(relative_signal_raw < 0, 0, relative_signal_raw)
...
# Strategy PnL
# If relative_signal is 0, it implies staying out, earning the RISK_FREE_RATE
strategy_pnl_relative = np.where(relative_signal == 0, RISK_FREE_RATE, relative_signal * actual_moves_aligned)
...
# For directional accuracy, only consider cases where a signal was actually given (not 0)
non_zero_signal_indices = relative_signal != 0
if np.sum(non_zero_signal_indices) > 0:
    new_dir_accuracy = np.mean(relative_signal[non_zero_signal_indices] == actual_direction_trimmed[non_zero_signal_indices])
else:
    new_dir_accuracy = 0.0 # No signals were given

print(f"--- De-Biased 'Relative Delta' Strategy (No Shorting) ---")
...
print(f"New Directional Accuracy (when signal is active): {new_dir_accuracy:.2%}")
...
if total_strategy_return_rel > total_market_return_trim:
    ...
    print("\nRESULT: Still underperforming (or matching). The model might just be outputting noise.")

# 6. Visualize
plt.figure(figsize=(12, 6))
plt.plot(np.cumsum(market_pnl_trimmed), label='Market Baseline', color='gray', alpha=0.5)
plt.plot(np.cumsum(strategy_pnl_relative), label='Relative Delta Strategy (No Short)', color='green')
